...
def Joueur(couleur, nom, nb_points, nb_faux_mvt, pos_pacman, pos_fantome, objets):
    ...
    """
    nouveaux_joueur={"couleur":couleur,
                     "nom":nom,
                     "nb_points":nb_points,
...
def joueur_from_str(description):
    ...
    """
    toto=description.split(";")
    nouveaux_joueur={"couleur":toto[0],
                     "nom":toto[10],
                     "nb_points":int(toto[1]),
                     "nb_faux_mvt":int(toto[2]),
                     "pos_pacman" : (int(toto[3]),int(toto[4])),
                     "pos_fantome":(int(toto[5]),int(toto[6])), 
                     "objets":{const.GLOUTON:int(toto[7]), 
                               const.IMMOBILITE:int(toto[8]), 
                               const.PASSEMURAILLE:int(toto[9])}}
    return nouveaux_joueur
chaine1="A;152;3;28;0;0;12;5;0;9;Greedy"
le_joueur=joueur_from_str(chaine1)
...
def get_objets(joueur):
    ...
    """
    return [x[0] for x in joueur["objets"].items() if x[1]!=0]
...
def get_duree(joueur,objet):
    ...
    """
    return joueur["objets"][objet] if objet in joueur["objets"] else 0
...
def maj_duree(joueur):
    ...
    Args:
        joueur (dict): le joueur considéré
    """
    for objets in joueur["objets"]:
        duree=get_duree(joueur, objets)
        if duree!=0:
            joueur["objets"][objets]=duree-1
